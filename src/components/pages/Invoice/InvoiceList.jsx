...
export default function InvoiceList() {
  ...
  const didProcess = useRef(false); // 👈 flag to prevent double insert

  useEffect(() => {
    if (location.state?.newInvoice && !didProcess.current) {
      setInvoices((prev) => [...prev, location.state.newInvoice]);
      didProcess.current = true; // mark as processed

      // clear location.state so it won't re-trigger on refresh
